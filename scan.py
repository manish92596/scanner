...
# def get_file_content_url(repo, path, commit_sha):
#     """Construct the URL to fetch the file content from a specific commit."""
#     return f"https://raw.githubusercontent.com/{repo}/{commit_sha}/{path}"

# def process_commit(repo, commit_sha):
#     """Process the files in a given commit to detect changes and vulnerabilities."""
#     commit_url = f"https://api.github.com/repos/{repo}/commits/{commit_sha}"
...
#                 threading.Thread(target=update_caches, args=(dummy_file_path,)).start()
#             else:
#                 logging.error(f"Failed to fetch file content for {filename}, status code: {file_response.status_code}")
#     else:
#         logging.error(f"Failed to fetch commit data: {response.status_code}")
...
#     except KeyError as e:
#         logging.error(f"KeyError: Missing key in JSON data - {e}")
#         return jsonify({"error": f"Missing key: {e}"}), 400
...
# @app.route('/home', methods=['GET'])
# def root():
#     """Simple root endpoint to check the status of the server."""
#     return jsonify({"status": "Success"}), 200
